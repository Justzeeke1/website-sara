...
const imageMap = import.meta.glob("/src/assets/sticker/*", {
  eager: true,
  import: "default",
});
...
const Sticker = () => {
  ...
  const handleClick = (title: string, preorder: boolean, idToClose?: string) => {
    const message = preorder
      ? `${t("stickers.order.preorderMessage", { defaultValue: "Vorrei preordinare" })} ${title}.`
      : `${t("stickers.order.orderMessage", { defaultValue: "Vorrei ordinare" })} ${title}.`;
      ...
      <section className="section-artistic py-12 md:py-0 md:h-72 lg:h-80 flex items-center"
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <h1 className="font-playwrite font-normal text-3xl sm:text-5xl font-bold text-foreground mb-4 sm:mb-6">
            {t("stickers.title", { defaultValue: "Sticker" })}
          </h1>
          <p className="text-base sm:text-xl text-muted-foreground max-w-3xl mx-auto">
            {t("stickers.subtitle", { defaultValue: "Collezione di sticker personalizzati" })}
          </p>
        </div>
      </section>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {stickers.map((sticker, index) => {
              ...
              const description =
                sticker.description?.[lang] || sticker.description?.["en"];
              const detailDescription =
                sticker.detailDescription?.[lang] || sticker.detailDescription?.["en"];

              const mainImage = Array.isArray(sticker.image)
                ? sticker.image[0]
                : sticker.image;

              const images = sticker.images?.length
                ? sticker.images
                : normalizeImages(sticker.image);
              ...
              return (
                ...
                >
                  <Dialog
                    open={openDialogId === sticker.id}
                    onOpenChange={(isOpen) =>
                      setOpenDialogId(isOpen ? sticker.id : null)
                    }
                  >
                        ...
                        {!sticker.available && !sticker.preorder && (
                          <div className="absolute inset-0 bg-black/50 flex items-center justify-center">
                            <Badge variant="destructive" className="text-white">
                              {t("stickers.unavailable", { defaultValue: "Non disponibile" })}
                            </Badge>
                          </div>
                        )}
                    ...
                    <DialogContent className="w-full max-w-[100vw] sm:max-w-[500px] md:max-w-2xl lg:max-w-4xl max-h-[90vh] overflow-y-auto">
                      <div className="grid md:grid-cols-2 gap-6">
                        ...
                        <div>
                          ...
                          <div className="mb-4">
                            {Object.entries(t("stickers.details", { returnObjects: true, defaultValue: {} })).map(
                              ([key, value]) => (
                          ...
                          <Button
                            ...
                            onClick={() =>
                              handleClick(title, sticker.preorder, sticker.id)
                            }
                          >
                            <ShoppingCart className="h-4 w-4 mr-2" />
                            {sticker.available
                              ? t("stickers.buy", { defaultValue: "Acquista" })
                              : sticker.preorder
                              ? t("stickers.preorder", { defaultValue: "Preordina" })
                              : t("stickers.unavailable", { defaultValue: "Non disponibile" })}
                          </Button>
                        </div>
                      </div>
                  ...
                  <CardContent className="p-6">
                    ...
                    <Button
                      ...
                      disabled={!sticker.available && !sticker.preorder}
                      onClick={() => handleClick(title, sticker.preorder, sticker.id)}
                    >
                      <ShoppingCart className="h-4 w-4 mr-2" />
                      {sticker.available
                        ? t("stickers.buy", { defaultValue: "Acquista" })
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-12">
            <h2 className="font-playwrite font-normal text-3xl text-foreground mb-6">
              {t("stickers.features.title", { defaultValue: "Caratteristiche" })}
            </h2>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            <div className="text-center">
              ...
              <h3 className="font-playwrite font-normal text-xl mb-3">
                {t("stickers.features.unique.title", { defaultValue: "Unici" })}
              </h3>
              <p className="text-muted-foreground">
                {t("stickers.features.unique.description", { defaultValue: "Ogni sticker è unico e personalizzato" })}
              </p>
            </div>
            <div className="text-center">
              ...
              <h3 className="font-playwrite font-normal text-xl mb-3">
                {t("stickers.features.durable.title", { defaultValue: "Resistenti" })}
              </h3>
              <p className="text-muted-foreground">
                {t("stickers.features.durable.description", { defaultValue: "Materiali di alta qualità e resistenti" })}
              </p>
            </div>
            <div className="text-center">
              <div className="w-16 h-16 bg-gradient-primary rounded-full flex items-center justify-center mx-auto mb-4">
              ...
              <h3 className="font-playwrite font-normal text-xl mb-3">
                {t("stickers.features.shipping.title", { defaultValue: "Spedizione" })}
              </h3>
              <p className="text-muted-foreground">
                {t("stickers.features.shipping.description", { defaultValue: "Spedizione veloce e sicura" })}
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <div className="bg-gradient-to-r from-primary/10 to-accent/10 rounded-3xl p-8">
            <h2 className="font-playwrite font-normal text-3xl text-foreground mb-4">
              {t("stickers.cta.title", { defaultValue: "Hai bisogno di sticker personalizzati?" })}
            </h2>
            <p className="text-lg text-muted-foreground mb-6">
              {t("stickers.cta.description", { defaultValue: "Contattaci per una commissione personalizzata" })}
            </p>
            <a href="/commissioni" className="btn-hero inline-flex items-center">
              {t("stickers.cta.button", { defaultValue: "Richiedi Commissione" })}
            </a>
          </div>
        </div>
      </section>
